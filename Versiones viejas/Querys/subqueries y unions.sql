...
-- del subquery decimos que es interno.

-- podemos tener un subquery en la cláusula WHERE, para filtrar los 
...
-- obtenemos los productos con precio mayor que el promedio de todos los artículos. Tanto la 
-- consulta externa como la interna se realizan sobre la misma tabla:
select 
...
-- Subquery en la cláusula FROM. Obtenemos el máximo, promedio y mínimo
...
-- Las subconsultas vistas hasta este momento son independientes: pueden 
-- ejecutarse por separado. Existen también las denominadas subconsultas
-- correlacionadas (correlated subqueries) que dependen de la consulta 
-- exterior. En este caso traemos los productos cuyo precio de lista
...
-- HAVING funciona como WHERE pero para los registros agrupados con GROUP BY

-- Subqueries en INSERTs:
-- Podemos insertar en una tabla el resultado de un query: En este caso la consulta exterior
-- será el insert y la interior la que devolverá el set de datos a insertar.
-- Supongamos que queremos tener una tabla con los clientes que compraron más de
-- cierto monto por factura:
